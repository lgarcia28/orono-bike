...
export function PromoPopup() {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-zinc-950/60 backdrop-blur-sm animate-fade-in">
      <div className="relative w-full max-w-lg bg-white border border-zinc-200 rounded-3xl overflow-hidden shadow-2xl text-zinc-900">
        
        {/* Botón de Cierre (✕) */}
        <button
          onClick={handleClose}
          className="absolute top-4 right-4 z-20 w-9 h-9 bg-white/80 hover:bg-white text-zinc-600 hover:text-zinc-950 rounded-full flex items-center justify-center border border-zinc-200 shadow-sm transition-all"
          title="Cerrar"
        >
          <X className="w-5 h-5" />
        </button>

        {/* Imagen de cabecera / Hero Banner */}
        <div className="relative h-44 sm:h-48 w-full overflow-hidden bg-zinc-100">
          ...
          <div className="absolute bottom-3 left-6 right-6">
            <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-amber-400 text-zinc-950 text-[10px] font-heading font-black tracking-wider uppercase rounded-full shadow-sm">
        ...
        <div className="p-6 sm:p-8 pt-2 space-y-4">
          {!isSubmitted ? (
            <>
              <div>
                <h3 className="text-xl sm:text-2xl font-heading font-black tracking-tight text-zinc-950 leading-tight">
                  {settings.title}
                </h3>
              ...
              <form onSubmit={handleSubmit} className="space-y-3 pt-1">
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full h-11 pl-10 pr-4 bg-zinc-50 border border-zinc-300 rounded-xl text-xs font-medium text-zinc-900 placeholder:text-zinc-400 focus:outline-none focus:ring-2 focus:ring-zinc-950 focus:bg-white focus:border-transparent transition-all"
                  />
                </div>

                <button
                  type="submit"
                  className="w-full h-11 bg-zinc-950 hover:bg-zinc-800 text-white font-heading text-xs font-black uppercase tracking-wider rounded-xl transition-all shadow-md active:scale-[0.99] flex items-center justify-center gap-2"
                >
            ...
            <div className="text-center space-y-4 py-2">
              <div className="w-14 h-14 bg-emerald-100 text-emerald-600 rounded-full flex items-center justify-center mx-auto border border-emerald-200 shadow-xs">
                <Check className="w-7 h-7" />
              </div>

              <div>
                <h3 className="text-lg sm:text-xl font-heading font-black text-zinc-950">
              ...
              <div className="flex items-center justify-center gap-2 bg-zinc-50 p-3 rounded-2xl border border-zinc-200 max-w-xs mx-auto">
                <span className="font-mono text-base sm:text-lg font-black tracking-widest text-zinc-950">
                  {settings.couponCode}
                </span>
                <button
                  onClick={handleCopyCode}
                  className="px-3 py-1.5 bg-zinc-950 hover:bg-zinc-800 text-white rounded-xl text-xs font-bold flex items-center gap-1.5 transition-colors shadow-xs"
                >
                  {copied ? (
                    <>
              ...
              <button
                onClick={handleClose}
                className="w-full h-11 bg-zinc-950 hover:bg-zinc-800 text-white font-heading text-xs font-bold uppercase tracking-wider rounded-xl transition-colors flex items-center justify-center gap-2 shadow-sm"
              >
